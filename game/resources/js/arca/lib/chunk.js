...
function Chunk(arca, id, i, j, k, size) {
    this.id = id
...
function setBlockLookups(arca) {
    solidLookup = arca.registry._solidityLookup
    opaqueLookup = arca.registry._opacityLookup
    objectMeshLookup = arca.registry._blockMeshLookup
    blockHandlerLookup = arca.registry._blockHandlerLookup
}
...
Chunk.prototype.updateMeshes = function () {
    if (this._terrainDirty) {
        this.arca.rendering.removeTerrainMesh(this)
        var mesh = this.mesh()
        if (mesh) this.arca.rendering.addTerrainMesh(this, mesh)
        this._terrainDirty = false
    }
