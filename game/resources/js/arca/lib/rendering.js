...
module.exports = function (arca, opts, canvas) {
    return new Rendering(arca, opts, canvas)
}
...
function Rendering(arca, _opts, canvas) {
    this.arca = arca
...
Rendering.prototype.initScene = function () {
    ...
    this.cameraTarget = this.arca.ents.createEntity(['position'])
    this.arca.ents.addComponent(this.cameraTarget, 'followsEntity', {
        entity: this.arca.playerEntity,
        offset: [0, this.arca.playerEyeOffset, 0],
    })
}
...
Rendering.prototype.highlightBlockFace = function (show, posArr, normArr) {
    ...
    if (show) {
        ...
        var dist = this._currentZoom + glvec3.distance(this.arca.getPlayerEyePosition(), posArr)
...
Rendering.prototype.addMeshToScene = function (mesh, isStatic) {
    ...
    if (mesh._currentarcaChunk || this._octree.dynamicContent.includes(mesh)) {
        return
    }
    var pos = mesh.position
    var chunk = this.arca.world._getChunkByCoords(pos.x, pos.y, pos.z)
    if (this._dynamicMeshOctrees && chunk && chunk.octreeBlock) {
        // add to an octree
        chunk.octreeBlock.entries.push(mesh)
        mesh._currentarcaChunk = chunk
    } else {
...
Rendering.prototype.removeMeshFromScene = function (mesh) {
    if (mesh._currentarcaChunk && mesh._currentarcaChunk.octreeBlock) {
        removeUnorderedListItem(mesh._currentarcaChunk.octreeBlock.entries, mesh)
    }
    mesh._currentarcaChunk = null
...
function updateDynamicMeshOctrees(self) {
    for (var i = 0; i < self._dynamicMeshes.length; i++) {
        ...
        var pos = mesh.position
        var prev = mesh._currentarcaChunk || null
        var next = self.arca.world._getChunkByCoords(pos.x, pos.y, pos.z) || null
...
function cameraObstructionDistance(self) {
    ...
    if (!_camBox) {
        ...
        _getVoxel = function (x, y, z) {
            return self.arca.world.getBlockSolidity(x, y, z)
        }
    }
...
function updateCamera(self) {
    ...
    var cpos = self.arca.ents.getPositionData(self.cameraTarget).renderPosition
    ...
    var id = self.arca.world.getBlockID(Math.floor(cam.x), Math.floor(cam.y), Math.floor(cam.z))
...
function checkCameraEffect(self, id) {
    ...
    if (id === 0) {
        ...
        var matId = self.arca.registry.getBlockFaceMaterial(id, 0)
        if (matId) {
            var matData = self.arca.registry.getMaterialData(matId)
