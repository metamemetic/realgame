...
module.exports = function (arca, opts) {
	return new CameraController(arca, opts)
}
...
function CameraController(arca, opts) {
	this.arca = arca
...
CameraController.prototype.updateForRender = function () {
	// input state
	var state = this.arca.inputs.state
	...
	var camrot = this.arca.rendering.getCameraRotation() // [x,y]
	var rotX = clamp(camrot[0] + dx, rotXcutoff)
	var rotY = (camrot[1] + dy) % (Math.PI * 2)
	this.arca.rendering.setCameraRotation(rotX, rotY)
