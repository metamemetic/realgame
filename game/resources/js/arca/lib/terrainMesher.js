...
function TerrainMesher() {
    ...
    */
    ...
    this.meshChunk = function (chunk, matGetter, colGetter, ignoreMaterials, useAO, aoVals, revAoVal) {
        ...
        var mats = matGetter || arca.registry.getBlockFaceMaterial
        var cols = colGetter || arca.registry._getMaterialVertexColor
        var ao = (useAO === undefined) ? arca.rendering.useAO : useAO
        var vals = aoVals || arca.rendering.aoVals
        var rev = isNaN(revAoVal) ? arca.rendering.revAoVal : revAoVal
...
function MeshBuilder() {
    ...
    this.build = function (chunk, meshdata, ignoreMaterials) {
        ...
        var mergeCriteria = function (mdat) {
            ...
            var url = arca.registry.getMaterialTexture(mdat.id)
            var alpha = arca.registry.getMaterialData(mdat.id).alpha
    ...
    function buildMeshFromSubmesh(submesh, name, mats, verts, inds) {
        ...
        var scene = arca.rendering.getScene()
    ...
    function getTerrainMaterial(matID, ignore) {
        if (ignore) return arca.rendering.flatMaterial
    ...
    function makeTerrainMaterial(id) {
        ...
        var matData = arca.registry.getMaterialData(id)
        if (matData.renderMat) return matData.renderMat
        // otherwise determine which built-in material to use
        var url = arca.registry.getMaterialTexture(id)
        var alpha = matData.alpha
        if (!url && alpha == 1) {
            // base material is fine for non-textured case, if no alpha
            return arca.rendering.flatMaterial
        }
        var mat = arca.rendering.flatMaterial.clone('terrain' + id)
        if (url) {
            var scene = arca.rendering.getScene()
...
function GreedyMesher() {
    ...
    function constructMeshDataFromMasks(i, d, u, v, len1, len2,
        ...
        var maskCompareFcn = (doAO) ? maskCompare : maskCompare_arcaO
        var meshColorFcn = (doAO) ? pushMeshColors : pushMeshColors_arcaO
    ...
    function maskCompare_arcaO(index, mask, maskVal, aomask, aoVal) {
        if (maskVal !== mask[index]) return false
        return true
    }

    function pushMeshColors_arcaO(colors, c, ao, aoValues, revAoVal) {
