...
module.exports = function (arca, dist) {
	...
	disc.material = arca.rendering.makeStandardMaterial('shadowMat')
	...
	return {
		...
		onAdd: function (eid, state) {
			state._mesh = arca.rendering.makeMeshInstance(disc, false)
		},
		...
		system: function shadowSystem(dt, states) {
			var cpos = arca.rendering.getCameraPosition()
			vec3.set(camPos, cpos.x, cpos.y, cpos.z)
			var dist = shadowDist
			for (var i = 0; i < states.length; i++) {
				var state = states[i]
				updateShadowHeight(state.__id, state._mesh, state.size, dist, arca)
			}
		},
		...
		renderSystem: function (dt, states) {
			...
			for (var i = 0; i < states.length; ++i) {
				var state = states[i]
				var rpos = arca.ents.getPositionData(state.__id).renderPosition
...
function updateShadowHeight(id, mesh, size, shadowDist, arca) {
	...
	if (b.resting[1] < 0) {
		...
		var pick = arca.pick(loc, down, shadowDist)
