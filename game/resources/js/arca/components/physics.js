...
module.exports = function (arca) {
	...
	return {
		...
		onAdd: function (entID, state) {
			state.body = arca.physics.addBody()
			// implicitly assume body has a position component, to get size
			var dat = arca.ents.getPositionData(state.__id)
			arca.ents.setEntitySize(state.__id, dat.width, dat.height, dat.width)
		},


		onRemove: function (entID, state) {
			arca.physics.removeBody(state.body)
		},
		...
		renderSystem: function (dt, states) {
			...
			var backtrack = - (arca._tickRate - dt) / 1000
			var pos = tempVec

			for (var i = 0; i < states.length; ++i) {
				var state = states[i]
				var id = state.__id
				var pdat = arca.ents.getPositionData(id)
				...
				if (arca.ents.cameraSmoothed(id)) {
